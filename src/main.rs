...
fn process_folder(
    ...
    let entries = match fs::read_dir(input_folder) {
        Ok(entries) => entries,
        Err(e) => {
    ...
    for entry_result in fs::read_dir(input_folder)? {
        let entry = match entry_result {
            Ok(entry) => entry,
            Err(e) => {
                eprintln!("Error saat membaca direktori: {}", e);
                continue;
            }
        };
    ...
    for entry_result in entries {
        ...
        if ["jpg", "jpeg", "png"].contains(&extension.to_lowercase().as_str()) {
            ...
            // Konversi gambar
            match covert_to_avif(path.to_str().unwrap(), output_path.to_str().unwrap()) {
                Ok(_) => {
                    processed_files += 1;
