...
use crate::api::admin::admin_series_handlers::{
    ...
    get_list_category_tags_handler, get_series_category_tags_handler, get_series_chapter_handler,
    repair_chapter_handler, update_existing_series_handler, upload_series_cover_image_handler,
};
...
fn admin_user_routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/users/update/{id}", patch(update_user_metadata_handler))
        .route(
...
fn admin_series_routes() -> Router<AppState> {
    Router::new()
        .route("/series/add", post(create_new_series_handler))
        .route("/series/chapter-list/{id}", get(get_series_chapter_handler))
        .route(
            "/series/delete/{id}/chapter/{chapter_number}",
            delete(delete_chapter_handler),
        )
...
pub fn admin_routes() -> Router<AppState> {
    admin_series_routes().merge(admin_user_routes())
}
