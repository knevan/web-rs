...
impl StorageClient {
    ...
    pub async fn delete_objects(&self, keys: Vec<String>) -> Result<()> {
        ...
        let objects_to_delete =
            objects_to_delete.context("Failed to build ObjectIdentifier objects")?;
        ...
        if !errors.is_empty() {
            ...
            return Err(anyhow::anyhow!("Some objects failed to delete from R2."));
        }

        // Get count of deleted objects
        let deleted_count = result.deleted.as_ref().map_or(0, |deleted| deleted.len());
        println!(
